...
import           Protolude                        hiding (replicateM, check)
...
headerBody = BC.pack <$> (listOf1 . elements . filter (`notElem` ("\r\n" :: P.String)) $ asciiChar)
...
genHeaders = do
    hs <- foldMap P.id <$> listOf1 genHeader
    return $ hs <> "\r\n"
...
main = do
    ret <- $(quickCheckAll)
    if ret then return ret else P.error "tests failed"
